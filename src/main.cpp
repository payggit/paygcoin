...
#if defined(NDEBUG)
# error "Paygcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x2fd7c329e92dc226c068eced3065571ada59d72793f1491f22001ea856d8d921");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Paygcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Paygcoin Signed Message:\n";
...
{
    // Paygcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 0 * COIN; //50 -> 100

    // Subsidy is cut in half every 840000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 1051200); // Paygcoin: 4 years, BlockHalving
...
static const int64 nTargetTimespan = 2 * 60; // Paygcoin: 2minutes, BlockDifficulty
static const int64 nTargetSpacing = 2 * 60; // Paygcoin: 2 minutes, BlockTime
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Paygcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Paygcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        pchMessageStart[0] = 0xfa; //This is Magic Number.
        pchMessageStart[1] = 0xc2; //
        pchMessageStart[2] = 0xb7; //
        pchMessageStart[3] = 0xdc; //
        hashGenesisBlock = uint256("0xca88ad45c74c9ccd9747cd83b5df9360f2a7aac474d9f28c8c40c5d6b58f335b");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "PAYGCOIN IS Number One Always";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 0 * COIN; //Edited
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04edb520c52a15aa77ffa8781a69c3de477c6ae8adab5b50271910796d6c1387aef90606933e20f55c056ee61f6fe8e41411ec619fe5043610839608f0aa6a293e") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1565871505;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2087262684;

        if (fTestNet)
        {
            block.nTime    = 1565871457;
            block.nNonce   = 397344313;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x39ca735aa1a717921208813d523e3f40a65e01e16f95b1850da23941da4a31e7"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc2, 0xb6, 0xdc }; // Paygcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PaygcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PaygcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PaygcoinMiner(CWallet *pwallet)
{
    printf("PaygcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("paygcoin-miner");
    ...
    try { loop {
        ...
        printf("Running PaygcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PaygcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PaygcoinMiner, pwallet));
}
